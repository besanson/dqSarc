...
def build_yaml(tier2: Path | None) -> str:
    lines = [
        ...
        # NB: deliberately no generated-date field — the artifact must be reproducible
        # so `--check` (CI) is stable across days. Dates live only in per-parameter
        # `computed_date` provenance, which is emitted solely when a dataset is present.
        "version: v1",
        "classes:",
    ]
